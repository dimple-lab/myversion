...
test_1_3_5() {
    ...
    ## Tests Start ##
egrep -w 'ESTABLISHED' /etc/iptables.up.rules >> $info_file 2>&1 || egrep -w 'ESTABLISHED' /etc/sysconfig/iptables >>$info_file 2>&1
[[ ! $@ ]] && result="Fail" || result="Pass" && echo "Config exist" >> $info_file && cat /etc/iptables.up.rules >> $info_file 2>&1 || cat /etc/sysconfig/iptables >> $info_file 2>&1
    ## Tests End ##
